...
class ButterflyGenePreprocessor(DataPreprocessor):
    ...
    def _process(self, pca_csv_path_suffix, verbose=False):
        ...
        @profile_exe_time
        def read_df():
            return pd.read_csv(os.path.join(self.input_dir, pca_csv_path_suffix), sep='\t', header=None, engine='pyarrow')
        ...
        def df_extract_states(df):
            return df.swifter.apply(extract_states)
        ...
        positions = list(df["Position"].values)
        column_dict = {i + 4: camids.values[i] for i in range(len(camids))}
        df = df.rename(columns = column_dict)
        states = states.rename(columns = {i: positions[i] for i in range(len(positions))})
    ...
    def _save_result(self, path) -> None:
        ...
        csv.write_csv(pa.Table.from_pandas(self.genotype_data["all_info"]), os.path.join(path, "all_info.csv"))
        csv.write_csv(pa.Table.from_pandas(self.genotype_data["states"], preserve_index=True), os.path.join(path, "states.csv"))
