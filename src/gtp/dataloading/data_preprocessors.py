...
class ButterflyGenePreprocessorOld(DataPreprocessor):
    ...
    def _process_with_pandas(
        self, pca_csv_path_suffix, verbose=False, process_max_rows=None
    ):
        ...
        match processor:
            case "pandas":
                ...
                self._process_with_pandas(
                    pca_csv_path_suffix,
                    verbose=False,
                    process_max_rows=process_max_rows,
                )
            case "polars":
                self._processor = "polars"
                self._process_with_polars(
                    pca_csv_path_suffix,
                    verbose=False,
                    process_max_rows=process_max_rows,
                )
    ...
    def _save_result(self, path) -> None:
        ...
        match self._processor:
            ...
            case _:
                raise NotImplementedError(
                    f"{self._processor} has not been implemented as a processor in {self.__class__.__name__}"
                )
