...
class GTP_Individual_Dataset(Dataset):
    def __init__(self, genotype_paths, phenotype_data):
        super().__init__()
        self.genotype_paths = genotype_paths
        self.phenotype_data = phenotype_data

    def __getitem__(self, idx):
        path = self.genotype_paths[idx]
        x = np.load(path)
    ...
    def __len__(self):
        return len(self.genotype_paths)
