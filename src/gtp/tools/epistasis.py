...
def detect_epistasis_pair_imm(snp1, snp2, phenotype, df):
    ...
    try:
        ...
        x = (snp1_tilde.dot(interaction_vector_tilde) / snp1_tilde.dot(snp1_tilde)) * (
            snp1_tilde
        )  # 2nd term in the v variable, using to breakdown the formula
        v = interaction_vector_tilde - x
        q2 = snp2_tilde - (
            ((snp1_tilde.dot(snp2_tilde)) / (snp1_tilde.dot(snp1_tilde))) * snp1_tilde
        )
        v = v - ((interaction_vector_tilde.dot(q2) / q2.dot(q2)) * q2)
        b3 = (v.dot(phenotype_tilde)) / (
            v.dot(v)
        )  # interactionn coefficient, referred as beta_3 in the paper

        # residual calculation
        residual = (
            phenotype_tilde
            - (snp1_tilde.dot(phenotype_tilde) / snp1_tilde.dot(snp1_tilde))
            * snp1_tilde
        )
    ...
    return (
        ...
        t_test,
        p_val,
    )  # returning the interaction coefficient,, t test value and p value
...
def detect_epistasis_imm(data, phenotype, alpha=0.05):
    ...
    for i, j in tqdm(
        idx_pairs,
        desc="Calculating Pairwise Epistasis",
        colour="green",
        position=0,
        leave=False,
    ):
        ...
        b3, t_test, p_val = detect_epistasis_pair_imm(
            interacting_snp_1, interacting_snp_2, phenotype, df=df
        )
    ...
    accept, corrected_pvals = ssm.fdrcorrection(
        np.asarray(all_p_values).flatten(), alpha=0.001
    )
    ...
    for i in range(0, len(corrected_pvals)):
        ...
        if corrected_pvals[i] < 0.001:
            print(
                " p-value = {0} \t adjusted p-value = {1} \t loci 1 = {2} \t loci 2 = {3} ".format(
                    p_value_locus[i][0],
                    corrected_pvals[i],
                    p_value_locus[i][1],
                    p_value_locus[i][2],
