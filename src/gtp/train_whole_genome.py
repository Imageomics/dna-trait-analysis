...
from gtp.evaluation import plot_attribution_graph, test, get_guided_gradcam_attr
from gtp.logger import Logger

from gtp.models.net import ConvNet, SoyBeanNet, LargeNet
...
from gtp.create_curve_from_sliding_window import create_curve
from gtp.experiments import get_experiment, get_all_gene_experiments
...
def train(args, tr_dloader, val_dloader, model, logger):
    ...
    for epoch in tqdm(range(args.epochs), desc="Training", colour="green"):
        ...
        for batch in val_dataloader:
            mse, rmse, best_diff, worst_diff = forward_step(model, batch, None, is_train=False, return_diff=True)
...
def get_args():
    ...
    parser.add_argument("--genome_folder", type=str, default="/local/scratch/carlyn.1/dna/vcfs/processed/genome")
    parser.add_argument("--phenotype_folder", type=str, default="/local/scratch/carlyn.1/dna/colors/processed")
    parser.add_argument("--split_data_folder", type=str, default="/home/carlyn.1/dna-trait-analysis/data")
    ...
    parser.add_argument("--output_dir", type=str, default="/local/scratch/carlyn.1/dna/results")
...
def load_data(args):
    ...
    """
    pca_path = os.path.join(args.phenotype_folder, f"{args.species}_{args.wing}_{args.color}", "data.csv")
    ...
    pca_data = pca_df.iloc[:, (1 + args.out_dims_start_idx):(1 + args.out_dims_start_idx + args.out_dims)].to_numpy()
    pca_data = pca_data[sort_idx]
    
    sorted_camids, compiled_data = collect_chromosome(args.genome_folder, args.species, args.chromosome)
    ...
    train_cams = np.load(os.path.join(args.split_data_folder, f"{args.species}_train.npy"))
    val_cams = np.load(os.path.join(args.split_data_folder, f"{args.species}_val.npy"))
    test_cams = np.load(os.path.join(args.split_data_folder, f"{args.species}_test.npy"))
...
if __name__ == "__main__":
    ...
    train_losses, val_losses, val_pearsons = train(args, train_dataloader, val_dataloader, model=model, logger=logger)
    ...
    plot_attribution_graph(model, train_dataloader, val_dataloader, test_dataloader, logger.outdir, ignore_train=True, mode="cam", ignore_plot=True, use_new=True)
    end_t = time.perf_counter()
    total_duration = end_t - start_t
